...
Key files:
- `lib/personas/types.ts`: Flattened TS interfaces for Persona.
- `lib/personas/schemas.ts`: Zod schemas for AI generation output and database storage (strict JSON validation).
- `lib/personas/prompt.ts`: Prompt builders for persona generation (single and batch) - generates flattened structure.
...
- `convex/schema.ts`: Convex tables; includes `personas` table with flattened schema and indexes.

### Persona generation agent
- Uses `generateObject` (non‑streaming) with `google('gemini-2.5-flash')` to return validated personas in **flattened format**.
- The AI generates personas directly in the flattened database-compatible structure (e.g., `profileFirstName`, `personalityOceanScoresOpenness`).
- Persona fields include OCEAN personality scores, flattened profile data, and pre‑ad context. The `preAdContextScenario`, `preAdContextCurrentActivity`, and `preAdContextEmotionalState` are derived by the model (not inputs).
...
- Table: `personas` uses a **flattened schema** for optimal database storage and querying.
- The AI now generates personas **directly in flattened format** (e.g., `profileFirstName`, `personalityOceanScoresOpenness`).
- No transformation needed - personas are ready to save after adding `id`, `audienceId`, and `userId`.
- Indexes:
  - `by_group` on `audienceGroup` for fast filtering.
  - `by_persona_id` on `personaId` for quick lookups.
  - `by_audience` on `audienceId` for session‑scoped lists.
  - `by_user` on `userId` for user-specific filtering.
- Saved personas include a stable `audienceGroup` (canonical ID), `audienceId` tag for the current generation session, and `userId` for the authenticated user.
- Note: Ad variants and simulation results are currently not persisted in the database (generated on-demand).

### Data structure
The codebase uses a **single flattened format** for personas:
- **AI Output** (`PersonaAIOutput`): Flattened structure without DB-specific fields (`id`, `userId`). Fields like `profileFirstName`, `personalityOceanScoresOpenness`, `preAdContextScenario`.
- **Database** (`Persona`): Same flattened structure with added `id`, `audienceId`, and `userId` fields.

**Key files**:
- `lib/personas/types.ts`: Defines `Persona` (flattened, for DB and components).
- `lib/personas/schemas.ts`: `PersonaSchema` (AI output), `FlattenedPersonaSchema` (DB storage with all fields).

**Data flow**:
1. AI generates flattened personas → validated by `PersonaSchema` (`PersonaAIOutput`)
2. Add DB-specific fields (`id`, `audienceId`, `userId`) → saved to Convex database
3. Database queries return flattened personas → components use flattened structure directly

**Authentication**:
- All persona generation actions require authentication.
- Uses `getAuthUserId(ctx)` from `@convex-dev/auth/server` to get the current user.
- Throws error if user is not signed in.

### Querying personas
Available Convex queries (return flattened `Persona[]`):
...
- `getByPersonaId({ personaId })`: Returns a single persona by its unique `personaId`, or `null` if not found.
...
// personas are in flattened format: persona.profileFirstName, persona.personalityOceanScoresOpenness, etc.
```
