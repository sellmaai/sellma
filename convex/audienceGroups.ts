...
import {
  buildAudienceOverviewPrompt,
  buildAudienceSubsegmentsPrompt,
  buildAudienceSuggestionBundlePrompt,
} from "../lib/personas/grouping/prompt";
import {
  AudienceOverviewSchema,
  AudienceSuggestionBundleSchema,
} from "../lib/personas/grouping/schemas";
  ...
  handler: async (_ctx, args) => {
    try {
      const { object } = await generateObject({
        ...
        schemaDescription:
          "Audience overview description and subsegment suggestions.",
      ...
      const { object: groupsObj } = await generateObject({
        ...
        prompt: buildAudienceSubsegmentsPrompt({
          text: args.text,
          count: args.count ?? 6,
        }),
      });
      ...
      const groups = Array.isArray((groupsObj as any).groups)
        ? (groupsObj as any).groups
        : [];
      const rawPercents = groups.map((g: { percent: number }) =>
        Math.max(1, Math.min(100, Math.round(Number(g.percent) || 0)))
      );
      ...
      if (sum === 0) {
        ...
        for (let i = 0; i < groups.length; i++) {
          rawPercents[i] = base + (i < remainder ? 1 : 0);
        }
        sum = 100;
      }
      ...
      const normalizedGroups = groups.map(
        (g: { percent: number }, i: number) => ({
          ...g,
          percent: rawPercents[i] ?? g.percent,
        })
      );
