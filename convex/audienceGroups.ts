...
export const suggestBundle = action({
  ...
  handler: async (_ctx, args) => {
    try {
      ...
      const rawPercents = groups.map((g: { percent: number }) => Math.max(1, Math.min(100, Math.round(Number(g.percent) || 0))));
      ...
      const normalizedGroups = groups.map((g: { percent: number }, i: number) => ({ ...g, percent: rawPercents[i] ?? g.percent }));
